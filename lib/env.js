...
const envSchema = z
  .object({
    ...
    ANONYMOUS_SESSION_SIGNING_SECRET: z.string().trim().min(1).optional(),
  })
...
const parsedEnv = envSchema.parse({
  ...
  ANONYMOUS_SESSION_SIGNING_SECRET: optionalTrimmedString(process.env.ANONYMOUS_SESSION_SIGNING_SECRET),
});
...
const anonymousSessionSigningSecret =
  parsedEnv.ANONYMOUS_SESSION_SIGNING_SECRET ||
  createHash('sha256')
    .update(`${parsedEnv.GROQ_API_KEY}:${parsedEnv.GEMINI_API_KEY}:likhle-anonymous-session`)
    .digest('base64url');
...
export function getAnonymousSessionSigningSecretFromEnv() {
  return anonymousSessionSigningSecret;
}
